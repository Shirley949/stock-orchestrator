...
⚠️ 快照空值语义（生产端隐性约定，消费方必读）：三表/报表行内 **False 是合法空占位
非缺失**（Sina 系源用 False 填「该科目本期无值」），且 bool 是 int 子类——数值消费方
判定/格式化前必须先排 bool（参考 gate_definitions._fmt_yi 的 isinstance(x, bool) 前置
排除，2026-09-01 WP1a 实证：现金流量表行内 '处置固定资产…净额': False）。
"""
